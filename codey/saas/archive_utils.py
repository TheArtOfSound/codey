...
def _archive_path_needs_sanitizing(path: str) -> bool:
    ...
    if any(char in normalized for char in {":", "?", "#"}):
        return True
    if _ARCHIVE_SECRET_VALUE_RE.search(normalized):
        return True
    path_obj = PurePosixPath(normalized)
    return path_obj.is_absolute() or any(part == ".." for part in path_obj.parts)


def dedupe_archive_path(archive_path: str, seen: set[str]) -> str:
    archive_path = (
        safe_archive_path(archive_path)
        if _archive_path_needs_sanitizing(archive_path)
        else archive_path
    )
