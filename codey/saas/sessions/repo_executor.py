...
class RepoExecutor:
    ...
    def real_diff(self) -> tuple[str, list[FileChange], str]:
        ...
        text = (_run(["git", "diff", "--cached"], self.workdir, timeout=60).stdoutTail or "")
        numstat = (_run(["git", "diff", "--cached", "--numstat"], self.workdir).stdoutTail or "")
        namestat = (_run(["git", "diff", "--cached", "--name-status"], self.workdir).stdoutTail or "")
        kinds = {}
    ...
    def commit(self, message: str) -> Optional[str]:
        ...
        status = _run(["git", "status", "--porcelain"], self.workdir).stdoutTail or ""
        if not status.strip():
            return self.head()
