...
from fastapi import APIRouter, Depends, HTTPException, UploadFile, File, Form, WebSocket, WebSocketDisconnect, Body, status
...
def _build_repo_grounding_context(
    ...
    if selected_files:
        ...
        for file_path in selected_files:
            try:
                snippet = read_text(file_path, max_chars=16000).strip()
            except Exception:
                continue
            if not snippet:
                continue
...
async def create_prompt_session(
    ...
    try:
        ...
        # Apply the caller's BYOK key (if configured) so generation runs on their
        # own capable model instead of the shared free fallback. resolve_model reads
        # this override; without it the /sessions path silently ignored saved keys.
        try:
            from codey.saas.intelligence.providers import set_byok_override
            set_byok_override(
                current_user.byok_provider,
                current_user.byok_api_key,
                current_user.byok_model,
            )
        except Exception:
            pass
        ...
        messages = [
            {
                ...
                "content": (
                    ...
                    "- EDITING AN EXISTING FILE: do NOT rewrite or reprint the whole file. "
                    "Output each change as a SEARCH/REPLACE block in EXACTLY this format, "
                    "including the FILE line:\n"
                    "FILE: path/to/file.ext\n"
                    "<<<<<<< SEARCH\n"
                    "<a few exact consecutive lines copied verbatim from the file shown above>\n"
                    "=======\n"
                    "<the replacement lines>\n"
                    ">>>>>>> REPLACE\n"
                    "Copy the SEARCH lines character-for-character from the repo context so "
                    "they match exactly. Use multiple blocks for multiple edits. Output a full "
                    "fenced file (with a FILE: path line) ONLY for brand-new files; never "
                    "reprint an existing file in full.\n"
                    ...
                    "# pip install X Y Z\n"
                    "\n"
                    "OUTPUT CONTRACT (REQUIRED \u2014 the commit pipeline depends on this):\n"
                    "- Immediately BEFORE every fenced code block, emit a line "
                    "`FILE: <repo-relative-path>` naming the exact file you create or edit "
                    "(e.g. `FILE: packages/qev-cli/bin/qev-workspace.js`).\n"
                    "- Use the REAL paths from the repository grounding above. Edit existing "
                    "files at their real paths; do NOT invent new top-level files when an "
                    "existing file should change.\n"
                    "- Each fenced block must contain the COMPLETE final contents of that file "
                    "(not a snippet, not a diff, no '... unchanged ...' elisions).\n"
                    "- Example:\n"
        ...
        try:
            ...
            try:
                from codey.saas.api._edits import parse_search_replace as _psr
                for _e in _psr(output):
                    _ep = _e.get("path")
                    if _ep:
                        _gen_files.setdefault(_ep, (_e.get("replace") or "").strip() + "\n")
            except Exception:
                pass
        except Exception:
            _gen_files = {}
...
async def get_session(
    ...
    return SessionDetailResponse(
        ...
        patch_receipt=(getattr(session, "patch_receipt", None) if isinstance(getattr(session, "patch_receipt", None), dict) else None),
...
async def commit_session(
    ...
    if not repo_connected:
        _chosen_repo_id = body.get("repo_id") if isinstance(body, dict) else None
        if _chosen_repo_id:
            from sqlalchemy import select as _sel_repo
            from codey.saas.models import Repository as _Repo_pick
            try:
                _picked = (await db.execute(
                    _sel_repo(_Repo_pick).where(
                        _Repo_pick.id == uuid.UUID(str(_chosen_repo_id)),
                        _Repo_pick.user_id == current_user.id,
                    )
                )).scalar_one_or_none()
            except Exception:
                _picked = None
            repo_connected = _coerce_non_empty_session_text(getattr(_picked, "full_name", None)) if _picked else None
            if repo_connected:
                session.repo_connected = repo_connected
                await db.flush()
    if not repo_connected:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="This run was not tied to a repository, so there is nowhere to commit it. Pick a repo to commit to, or re-run with a repo selected for a repo-aware change.",
        )
    ...
    branch = f"codey/session-{str(session.id)[:8]}-{uuid.uuid4().hex[:6]}"
    try:
        try:
        ...
        # Apply SEARCH/REPLACE edits surgically against the real cloned files so an
        # edit can never delete the rest of a file. Falls back to the guard below.
        try:
            from codey.saas.api._edits import parse_search_replace as _parse_sr, apply_search_replace as _apply_sr
            import os as _os_sr
            _sr_src = _coerce_non_empty_session_text(getattr(session, "output_summary", None)) or ""
            _sr_by_path = {}
            for _e in _parse_sr(_sr_src):
                _rp = _e.get("path")
                if _rp:
                    _sr_by_path.setdefault(_resolve_repo_path(str(_rp).lstrip("/")), []).append(_e)
            for _rp, _el in _sr_by_path.items():
                _rfull = _os_sr.path.join(ex.workdir, _rp)
                if _os_sr.path.isfile(_rfull):
                    _ro = open(_rfull, encoding="utf-8", errors="ignore").read()
                    _rn, _rap, _rpb = _apply_sr(_ro, _el)
                    if _rap:
                        generated_files[_rp] = _rn
        except Exception:
            pass
        # Never commit an empty placeholder left by an edit we could not apply.
        for _gp0 in list(generated_files.keys()):
            if (generated_files.get(_gp0) or "").strip() == "":
                del generated_files[_gp0]
        # Guardrail: never overwrite an existing file with a drastically shorter
        # rewrite. The model sometimes truncates a file it was asked to edit; an
        # honest abort beats silently deleting the user's code.
        import os as _os_guard
        _trunc_skipped = []
        for _gp in list(generated_files.keys()):
            _gfull = _os_guard.path.join(ex.workdir, str(_gp).lstrip("/"))
            if _os_guard.path.isfile(_gfull):
                try:
                    _orig_lines = open(_gfull, encoding="utf-8", errors="ignore").read().splitlines()
                except Exception:
                    continue
                _new_lines = (generated_files.get(_gp) or "").splitlines()
                if len(_orig_lines) >= 25 and len(_new_lines) < 0.6 * len(_orig_lines):
                    _trunc_skipped.append(f"{_gp} ({len(_new_lines)} vs {len(_orig_lines)} lines)")
                    del generated_files[_gp]
        if not generated_files:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail={
                    "message": "Not committed - the generated change would have deleted most of an existing file (a truncated rewrite). Re-run with a narrower prompt. You were not charged.",
                    "truncated_files": _trunc_skipped,
                },
            )
        ...
        try:
            async with _httpx.AsyncClient(timeout=20.0) as _hc:
                ...
                if _resp.status_code < 300:
                    ...
                    # A PR for this branch may already exist (re-commit of the same
                    # session) — look it up rather than reporting a spurious failure.
                    _owner = repo_connected.split("/")[0]
                    _look = await _hc.get(
                        f"https://api.github.com/repos/{repo_connected}/pulls",
                        headers={"Authorization": f"Bearer {gh_token}", "Accept": "application/vnd.github+json"},
                        params={"head": f"{_owner}:{branch}", "state": "open"},
                    )
                    if _look.status_code < 300 and _look.json():
                        pr_url = _look.json()[0].get("html_url")
                    else:
                        try:
                            _body = _resp.json()
                            pr_error = ((_body.get("errors") or [{}])[0].get("message")
                                        or _body.get("message") or f"GitHub returned {_resp.status_code}")
                        except Exception:
                            pr_error = f"GitHub returned {_resp.status_code}"
        except Exception as _pe:
            pr_error = f"Could not reach GitHub to open the PR: {str(_pe)[:140]}"
    finally:
    ...
    if pr_url:
        _msg = f"Committed {_commit_after} to {branch} and opened a pull request."
    elif pr_error:
        _msg = (f"Committed {_commit_after} and pushed branch {branch}, but opening the PR "
                f"failed: {pr_error}. You can open it manually on GitHub.")
    else:
        _msg = f"Committed {_commit_after} and pushed branch {branch}. Open a PR on GitHub to merge."
    return CommitResponse(
        ...
        branch=branch,
        files_changed=_files_changed[:50],
        verified=_verified,
        pr_error=pr_error,
    )
