...
FALLBACK_TOKEN_BUDGETS = (2048,)  # keep fallbacks large enough for a real file; sub-1k budgets produced truncated half-files
...
def _is_rate_limit_or_quota_error(error: BaseException) -> bool:
...
def _should_retry_with_lower_token_budget(error: BaseException) -> bool:
    text = str(error).lower()
    return any(
        marker in text
        for marker in (
...
def _fallback_token_budgets(max_tokens: int) -> list[int]:
    """Return descending fallback token budgets bounded by the requested budget."""
    requested = max(1, int(max_tokens or 1))
    budgets = [requested]
    budgets.extend(budget for budget in FALLBACK_TOKEN_BUDGETS if budget < requested)
...
def _provider_has_key(provider: str) -> bool:
    ...
    if cfg is None:
        return False
    return _coerce_non_empty_provider_env(cfg["key_env"]) is not None
    ...
    **kwargs: Any,
    ...
    try:
        ...
        if _is_rate_limit_or_quota_error(e):
            logger.warning(
                "Primary model %s/%s unavailable due to rate/quota limits, trying fallbacks",
            ...
            for fb in FALLBACK_MODELS:
                ...
                if not _provider_has_key(fallback_provider):
                    logger.warning(
                        "Fallback %s/%s skipped because provider key is not configured",
                        _redact_provider_error(fallback_provider),
                        _redact_provider_error(fallback_model),
                    )
                    continue

                # Back off so a transient per-minute rate-limit window can recover
                # (the first fallback is the primary provider's model).
                await asyncio.sleep(2.0)
                for fallback_max_tokens in _fallback_token_budgets(max_tokens):
                    try:
                        return await _call_model_once(
                            fallback_provider,
                            fallback_model,
                        ...
                        logger.warning(
                            "Fallback %s/%s failed at max_tokens=%s: %s",
                            _redact_provider_error(fallback_provider),
                            _redact_provider_error(fallback_model),
                            fallback_max_tokens,
                            _redact_provider_error(fb_err),
                        )
                        if not _should_retry_with_lower_token_budget(fb_err):
                            break
                        continue

            raise RuntimeError(
                "AI providers are temporarily unavailable: primary provider is "
                "rate-limited or out of quota, and configured fallbacks could not "
                "complete the request. Add provider credits or configure a paid "
                "production LLM key."
            ) from e
        raise
