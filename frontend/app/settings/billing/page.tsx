...
function BillingPageContent() {
  ...
  const cleanBillingUrl = useCallback((planId?: string | null) => {
  ...
  }, [referralId, selectedPlanId]);

  const finalizeSubscription = useCallback(async (subscriptionId: string) => {
  ...
  }, [cleanBillingUrl, clearCheckout, refreshUser, reloadBillingData]);

  const beginSubscription = useCallback(async (planId: string) => {
    setActionLoading(true);
    setError(null);
    setMessage(null);
  ...
  }, [beginSubscription, selectedPlanId, setupSuccess, handledSetupResume]);
  ...
  }, [finalizeSubscription, returnedSubscriptionId, paymentSuccess, handledPaymentResume]);
