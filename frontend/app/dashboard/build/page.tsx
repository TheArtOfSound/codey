...
function buildErrorDetail(err: unknown, fallback: string): string {
  const detail = (err as { detail?: string } | null | undefined)?.detail;
...
export default function BuildPage() {
  ...
  // Surfaced failure from /build/start | /build/plan | /build/approve so a
  // failed request shows a message instead of a silently stopped spinner.
  const [submitError, setSubmitError] = useState<string | null>(null);
  // GitHub push pending state for the COMPLETE-state action button.
  ...
  async function handleStartPlanning() {
    ...
    try {
      ...
      const msg = buildErrorDetail(err, "Could not start planning. Please try again.");
      setSubmitError(msg);
      addToast(msg, "error");
    } finally {
      setSubmitting(false);
  ...
  async function handleBuildWithAnswers() {
    ...
    try {
      ...
      const msg = buildErrorDetail(err, "Could not create the build plan. Please try again.");
      setSubmitError(msg);
      addToast(msg, "error");
    } finally {
  ...
  async function handleApprove() {
    ...
    try {
      ...
      const msg = buildErrorDetail(err, "Could not start the build. Please try again.");
      setSubmitError(msg);
      addToast(msg, "error");
    } finally {
      setApproving(false);
  ...
  async function handleDownloadZip(): Promise<boolean> {
    ...
    try {
      ...
      addToast(
        buildErrorDetail(err, "Could not prepare the download. Please try again."),
        "error"
      );
      return false;
    }
  ...
  // NOTE: Build-mode projects are delivered as a downloadable ZIP. There is
  // no backend route that pushes a *build* project to GitHub: github_repo_url
  // is never populated and POST /github/pr operates on a coding session with a
  // connected repo, which the build flow does not create. So the best real
  // action here is to deliver the ZIP and tell the user to push it manually.
  async function handlePushToGitHub() {
    if (!projectId || pushingToGitHub) return;
    setPushingToGitHub(true);
    try {
      const ok = await handleDownloadZip();
      if (ok) {
        addToast(
          "Direct GitHub push isn\u2019t available for build-mode projects yet \u2014 your ZIP is downloading, push it to a repo from there.",
          "info"
        );
      }
    } finally {
  ...
  if (buildState === "DESCRIBE") {
    return (
      <div className="mx-auto max-w-5xl space-y-8">
        ...
        {/* Submit error */}
        {submitError && (
          <div className="flex items-start gap-2 rounded-xl border border-codey-red/30 bg-codey-red/5 px-4 py-3 text-sm text-codey-red">
    ...
    return (
      <div className="flex h-[calc(100vh-8rem)] flex-col gap-0 overflow-hidden">
        ...
        <div className="mt-px flex items-center gap-6 rounded-b-xl border border-t-0 border-codey-border bg-codey-card px-5 py-2.5">
          ...
          <div className="h-4 w-px bg-codey-border" />
          {/* Escape hatch: never strand the user in the IDE if the build
              stalls or fails. Always available, emphasised on trouble. */}
          <button
            onClick={handleReset}
            className={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs transition-colors ${
              !wsConnected || streamFallbackMessage
                ? "border-codey-red/40 text-codey-red hover:bg-codey-red/10"
      ...
      <div className="mx-auto max-w-3xl space-y-8">
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          <button
            ...
            className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-codey-border bg-codey-card px-6 py-4 text-sm font-medium text-codey-text transition-colors hover:bg-codey-card-hover disabled:cursor-not-allowed disabled:opacity-50"
          >
            {pushingToGitHub ? (
              <>
