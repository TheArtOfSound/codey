...
class ApiClient {
  ...
  setToken(token: string | null) {
    ...
    if (typeof window !== "undefined") {
      try {
        if (token) window.localStorage.setItem("codey_token", token);
        else window.localStorage.removeItem("codey_token");
      } catch {
  ...
  // Rehydrate the in-memory token from localStorage so a reload / new tab stays
  // logged in even if the proxy session cookie did not survive.
  loadStoredToken(): string | null {
    if (this.token) return this.token;
    if (typeof window !== "undefined") {
      try {
        const stored = window.localStorage.getItem("codey_token");
  ...
  async commitSession(id: string): Promise<{ session_id: string; credits_charged: number; message: string; pull_request_url?: string | null; branch?: string | null; files_changed?: string[]; verified?: boolean; pr_error?: string | null }> {
    return this.request<{ session_id: string; credits_charged: number; message: string; pull_request_url?: string | null; branch?: string | null; files_changed?: string[]; verified?: boolean; pr_error?: string | null }>(
      `/sessions/${id}/commit`,
      { method: "POST" }
    );
  }
